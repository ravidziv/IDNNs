...
def testDist():
    pyxs= np.random.rand(10000000,2)
    dx = np.random.randint(10000000, size=1000)
    s_pyxs = pyxs[dx,:]

    rows_sum = pyxs.sum()
    pyxs_new = pyxs / (rows_sum)
    X = pyxs_new[:,0]
    Y = pyxs_new[:,1]
    MI_est = mutual_information_2d(X.ravel(), Y.ravel())

    rows_sum = s_pyxs.sum()
    pyxs_new = s_pyxs / (rows_sum)
    X = pyxs_new[:, 0]
    Y = pyxs_new[:, 1]
    MI_est_s = mutual_information_2d(X.ravel(), Y.ravel())
    import entropy_estimators as ee
    Y = [[y] for y in Y]
    X = [[x] for x in X]
    #ee.mi(X,Y)
    print MI_est, MI_est_s,ee.mi(X,Y)
    #cols_sum = pyxs_new.sum(axis=0)
    print pyxs_new.shape
