...
def IB_iteration(p_t_x, PXs, PYgivenXs, beta, num_of_iterations):
...
def mean_DKL(beta, p_t_x, PXs, PYgivenXs, num_of_iterations):
    """Return The mean D_KL between p(x|t) and p(x|t) based on the IB"""
    probTgivenXs, PYgivenTs_update = IB_iteration(p_t_x, PXs, PYgivenXs, beta, num_of_iterations)
...
def calc_infomration_for_layer(data, bins, label, b, b1, len_unique_a, pys, pxs, py_x,Xs):
    ...
    b2 = np.ascontiguousarray(binning_data).view(
        np.dtype((np.void, binning_data.dtype.itemsize * binning_data.shape[1])))
    unique_array, unique_inverse_t, unique_counts = \
        np.unique(b2, return_index=False, return_inverse=True, return_counts=True)
    #unique_counts is the number of Ts in this layer
    p_ts = unique_counts / float(sum(unique_counts))
    #pxy_given_T[:, 0] is  p(x|T) and pxy_given_T[:, 1] is p(y|T)
    pxy_given_T = np.array(
        [calc_probs(i, unique_inverse_t, label, b, b1, len_unique_a) for i in range(0, len(unique_array))]
    )
    local_IXT, local_ITY = process_and_calc_information_inner(pxy_given_T[:, 0], pxy_given_T[:, 1], p_ts, pys, pxs, py_x)
    est_IXT, est_ITY = estimate_Information(Xs,label, binning_data )
    return local_IXT, local_ITY, est_IXT, est_ITY
...
def get_information_for_epoch(iter_index, ws_iter_index, bins, label, b, b1, len_unique_a, pys, pxs, py_x,x):
    ...
    iter_infomration = np.array(
        [calc_infomration_for_layer(ws_iter_index[i], bins, label, b, b1, len_unique_a, pys, pxs, py_x,x) for i in range(0, len(ws_iter_index))])
...
def get_infomration(ws, x, label,num_of_bins):
    ...
    #create unique rows for the neurons output
    ...
    infomration = np.array(Parallel(n_jobs=NUM_CORES)(delayed(get_information_for_epoch) (
        i, ws[i], bins, label, b, b1, len(unique_a), p_ys, p_xs, p_y_given_x,x) for i in range(len(ws))))
    #For only the final epoch
    #infomration = np.array(get_information_for_epoch(-1, ws[-1], bins, label, b, b1, len(unique_a), p_ys, p_xs, p_y_given_x,x))
    return infomration
...
def train_network(layerSize, num_of_ephocs, learning_rate_local, batch_size, indexes, save_grads, data_sets):
    """Train the nework"""
    ...
    with tf.Graph().as_default():
        ...
        correct_prediction = tf.equal(tf.argmax(logits, 1), tf.argmax(labels, 1))
        accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
        batch_points = np.rint(np.arange(0, data_sets.train.num_examples+1,batch_size)).astype(dtype=np.int32)
        print ('Number of batch_points in batch  - ' , len(batch_points))
        init = tf.global_variables_initializer()
        ws, ws_test,ws_train,ws_ps, train_pred, test_pred,loss_func_test,loss_func_train,var_grad_val, l1_norm, l2_norm = [], [],[],[],[], [],[],[],[], [],[]
        ...
        with tf.Session() as sess:
            ...
            for j in range(0, num_of_ephocs):
                ...
                for i in xrange(0, len(batch_points) - 1):
                    batch_xs = data_sets.train.images[batch_points[i]:batch_points[i + 1],:]
                    batch_ys = data_sets.train.labels[batch_points[i]:batch_points[i + 1],:]
                    optimizer.run({x: batch_xs, labels: batch_ys})
                    epochs_grads.append(sess.run(grads, feed_dict={x: batch_xs, labels: batch_ys}))
                #Calculate all the measures if we are in epoch to insert
                if j in indexes:
    ...
    return ws, test_pred, train_pred, loss_func_test, loss_func_train,var_grad_val,l1_norm,l2_norm, flatted_list, ws_ps
...
def train_and_calc_information(calc_information, data_sets, sample_size, layerSize, num_of_ephocs, name_to_save,
    ...
    ws, test_pred, train_error, loss_func_test, loss_func_train, var_grad_val, l1_norm, l2_norm, flatted_list, ws_ps = \
        train_network(layerSize, num_of_ephocs, learning_rate, batch_size, indexes, save_grads, data_sets)
    information_all = get_infomration(ws, data_sets.test.images,data_sets.test.labels,num_of_bins) if calc_information ==1 else 0
...
def select_network_arch(type_net):
    ...
    if type_net == '1':
        ...
        layers_sizes = [[400,100, 40, 30, 30 ,25,20]]
    else:
...
def train_and_calc_information_all_networks(type_net, name_to_store, new_index, batch_size, learning_rate, num_ephocs, num_of_repeats, data_name,
    ...
    train_samples = np.linspace(1, 100, 199)[[[x*2-2 for x in index] for index in new_index]]
    ...
    #The jobs parmas
    params = {'samples_len' : len(train_samples), 'num_of_disribuation_samples':num_of_disribuation_samples, 'layersSizes': layers_sizes, 'numEphocs':num_ephocs, 'batch': batch_size,
    ...
    params['train_samples'], params['CPUs'], params['nameSave'] , params['directory'] = train_samples,  NUM_CORES,name_to_store, name_to_save
    for i in params:
        print i, params[i]
    params['epochsInds'] = epochs_indexes
    #The weights
    weights = [[[[None] for k in range(len(train_samples))] for j in range(len(layers_sizes))]for i in range(num_of_repeats)] if save_ws ==1 else 0
    #the variance of the gradints
    var_grads = [[[[None] for k in range(len(train_samples))] for j in range(len(layers_sizes))]for i in range(num_of_repeats)] if save_grads ==1 else 0
    information = np.zeros([num_of_repeats, len(layers_sizes), len(train_samples), len(epochs_indexes), max_size + 1, 2]) if calc_information ==1 else 0
    #information_each_neuron = [[[[None] for k in range(len(train_samples))] for j in range(len(layers_sizes))]for i in range(num_of_repeats)]
    test_error, train_error = np.zeros([num_of_repeats, len(layers_sizes), len(train_samples), len(epochs_indexes)]), np.zeros([num_of_repeats, len(layers_sizes), len(train_samples), len(epochs_indexes)])
    loss_train, loss_test = np.zeros([num_of_repeats, len(layers_sizes), len(train_samples), len(epochs_indexes)]), np.zeros([num_of_repeats, len(layers_sizes), len(train_samples), len(epochs_indexes)])
    l1_norms, l2_norms = np.zeros([num_of_repeats, len(layers_sizes), len(train_samples), len(epochs_indexes)]), np.zeros([num_of_repeats, len(layers_sizes), len(train_samples), len(epochs_indexes)])
    #run and calculate the infomration of the network in parallel or not
    if run_in_parallel ==1:
        infomration = Parallel(n_jobs=NUM_CORES)(delayed(run_specific_network)
                                      (calc_information, i,j,k , train_samples[i], layers_sizes[j], num_ephocs,name_to_save,learning_rate,batch_size,epochs_indexes,save_ws,num_of_bins)
                                        for i in range(len(train_samples)) for j in range(len(layers_sizes)) for k in range(num_of_repeats))
    else:
        infomration= [run_specific_network(
            calc_information, i, j, k, data_sets, train_samples[i], layers_sizes[j], num_ephocs, name_to_save,
        ...
        for j in range(len(layers_sizes)):
            for k in range(num_of_repeats):
                ...
                if calc_information ==1:
                    org_information = loca_inf_all[:, :,0:2]
                    est_information = loca_inf_all[:, :, 2: ]
                ...
                if save_ws == 1: weights[k][j][i] = flatted_list_ws
                if save_grads == 1: var_grads[k][j][i] = var_grads
    #Save everything into file
    vars_list = {'information': information, 'test_error': test_error, 'train_error': train_error,
                 'loss_test': loss_test, 'loss_train': loss_train,'params': params, 'var_grads': var_grads
                    ,'l1_norms': l1_norms,'weights': weights}
    saveData(vars_list, name_to_save)
    print ('Finished')

def parse_and_run_networks():
    #he paramters that you can pass to the method
    parser = argparse.ArgumentParser()
    parser.add_argument('-start_samples', '-ss', dest="start_samples",default=1, type=int, help = 'The number of the first sample that we calculate the information')
    parser.add_argument('-batch_size', '-b', dest="batch_size",default=2544, type=int, help = 'The size of the batch')
    parser.add_argument('-learning_rate', '-l', dest="learning_rate",default=0.0004, type=float, help= 'The learning rate of the network')
    parser.add_argument('-num_repeat', '-r', dest="num_of_repeats",default=1, type=int, help= 'The number of times to run the network')
    parser.add_argument('-num_epochs', '-e', dest="num_of_ephocs",default=1400, type=int, help = 'max number of epochs')
    parser.add_argument('-net', '-n', dest="net_type",default='6', help = 'The architecture of the networks')
    parser.add_argument('-inds', '-i', dest="inds",default='[85]', help = 'The percent of the training data')
    parser.add_argument('-name', '-na', dest="name",default='r' , help = 'The name to save the results')
    parser.add_argument('-d_name', '-dna', dest="data_name",default='var_u', help = 'The dataset name')
    parser.add_argument('-num_samples', '-ns', dest="num_of_samples",default=1800, type = int,help = 'The max number of indexes for calculate information' )
    parser.add_argument('-num_of_disribuation_samples', '-nds', dest="num_of_disribuation_samples",default=1, type = int, help = 'S')
    parser.add_argument('-save_ws', '-sws', dest="save_ws",default=False, help = 'if we want to save the weights of the network')
    parser.add_argument('-calc_information', '-cinf', dest="calc_information",default=1, help = '1 if we want to calculate the MI in the network')
    parser.add_argument('-save_grads', '-sgrad', dest="save_grads",default=False, help = 'Save tge gradients in the network')
    parser.add_argument('-run_in_parallel', '-par', dest="run_in_parallel",default=False, help = 'If we want to run all the networks in parallel mode')
    parser.add_argument('-num_of_bins', '-nbins', dest="num_of_bins",default=50, help = 'The nuber of bins that we divide the neurons output')
