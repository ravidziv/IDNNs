        ...
        with tf.Session() as sess:
            ...
            for j in range(0, num_of_ephocs):
                ...
                if j in indexes:                
                    ...
                    var_grad_val.append(sess.run(grads, feed_dict={x: X_train, y_true: labels_train}))
                    #l1_norm.append(sess.run(weightsModel))
                    ...
                    l2_norm.append(LA.norm(np.array(flatted_list), 2))

                    #print (np.array(sess.run(grads, feed_dict={x: X_train, y_true: labels_train})))

    return ws, test_pred, train_pred, X_test, X_train, loss_func_test, loss_func_train,var_grad_val,l1_norm,l2_norm
...
def calcAndRun(F_org, y_org, sampleSize, layerSize, F, y_t, input_size, num_of_ephocs, name_to_save, 
    ...
    t = time.time()
    ws, test_pred, train_error, X_test, X_train,loss_func_test, loss_func_train,var_grad_val,l1_norm,l2_norm  = runNet(F_org, y_org, sampleSize, layerSize, F, y_t, input_size,
    ...
    return information_all, test_pred, train_error, loss_func_test, loss_func_train,ws,var_grad_val,l1_norm,l2_norm
...
def runNetParallel(i,j,k , F,y, samples, layers_sizes,input_size, num_of_ephocs,name_to_save,learning_rate,batch_size,
    ...
    #return [i], [j,j], [k,k,k]
    loca_inf_all , local_erorr, local_train,loss_func_test, loss_func_train,ws,var_grad_val,l1_norm,l2_norm = \
    ...
    return loca_inf_all , local_erorr, local_train,loss_func_test, loss_func_train,ws,var_grad_val,l1_norm,l2_norm
    ...
    for i in range(len(samples)):
        for j in range(len(layers_sizes)):
            for k in range(num_of_repeats):            
                ...
                loca_inf_all , local_erorr, local_train,loss_func_test, loss_func_train,ws,c_var_grad_val,l1_norm,l2_norm    = infomration[index] 
                print ('-------------------')
                ...
                print (per_neuron_information.shape)
